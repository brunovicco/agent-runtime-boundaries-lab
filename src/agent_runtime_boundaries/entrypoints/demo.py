...
from agent_runtime_boundaries.application.service import (
    build_identity,
    build_specialist_request,
    execute_delegation,
)
from agent_runtime_boundaries.domain.contracts import ReviewCommand, WorkflowPhase
...
async def execute_once(
    ...
    response, reused = await execute_delegation(
        ledger=ledger, specialist=specialist, request=request
    )
