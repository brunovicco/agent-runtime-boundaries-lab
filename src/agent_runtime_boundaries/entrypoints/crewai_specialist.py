...
observability = configure_observability(
    service_name="crewai-risk-specialist",
...
app = FastAPI(title="CrewAI Risk Specialist", version="0.2.0", lifespan=lifespan)
app.include_router(build_a2a_router(specialist=specialist, observability=observability))


@app.get("/health")
async def health() -> dict[str, str]:
    """Describe the CrewAI runtime boundary without exposing credentials."""
    return {
        "status": "ok",
        "runtime": "crewai",
        "composition": "crew",
